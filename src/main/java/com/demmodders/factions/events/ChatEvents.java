...
public class ChatEvents {
    ...
    public static void checkFactionChat(ServerChatEvent e){
        ...
        if (FactionConfig.factionSubCat.chatFName && !FactionManager.WILDID.equals(factionID)){
            e.setComponent((new TextComponentString(TextFormatting.DARK_GREEN + "[" + fMan.getFaction(factionID).name + "]")).appendSibling(e.getComponent()));
        }

        if(player.factionChat != FactionChatMode.NORMAL) {
            // Cancel event
            e.setCanceled(true);

            // Make message
            ITextComponent message = (new TextComponentString(TextFormatting.DARK_GREEN + "[Faction Chat]")).appendSibling(e.getComponent());
